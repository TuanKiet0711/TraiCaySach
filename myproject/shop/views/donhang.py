...
def _build_page_numbers(cur, total_pages, span=2, edge=1):
    if total_pages <= (2 * edge + 2 * span + 1): return list(range(1, total_pages + 1))
    pages=set(); [pages.add(i) for i in range(1, edge+1)]; [pages.add(i) for i in range(total_pages-edge+1, total_pages+1)]
    for i in range(cur-span, cur+span+1):
        if 1 <= i <= total_pages: pages.add(i)
    ordered=sorted(pages); result=[]; prev=None
    for p in ordered:
        if prev is not None and p-prev>1: result.append("...")
        result.append(p); prev=p
    return result

def _account_label(acc):
    if not acc: return "—"
    return acc.get("ho_ten") or acc.get("ten") or acc.get("ten_dang_nhap") or acc.get("username") or acc.get("email") or "—"
...
def orders_list(request):
    ...
    context = {
        "items": items, "q": q, "status": status, "pay": pay, "sort": sort,
...
def order_create(request):
    products = list(san_pham.find({}, {"ten": 1, "ten_san_pham": 1}).sort("_id", -1).limit(100))
    accounts = list(tai_khoan.find({}, {"ho_ten": 1, "ten": 1, "email": 1, "username": 1, "ten_dang_nhap": 1}).sort("_id", -1).limit(100))
    for p in products: p["id"] = str(p["_id"]); p["ten"] = _product_label(p)
    for a in accounts: a["id"] = str(a["_id"]); a["ten"] = _account_label(a)
